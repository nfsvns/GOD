...
public class AccountRestController {
	...
	public ResponseEntity<String> sendEmail(@RequestBody Map<String, String> requestData) {
		...
		if (account != null) {
			...
			MailInfo mail = new MailInfo(account.getEmail(), "Mật khẩu mới",
					"Hi " + account.getUsername() + ", mật khẩu mới của bạn là: " + newPassword);
			try {
				mailerService.send(mail);
				return ResponseEntity.ok("Mật khẩu mới đã được gửi đến địa chỉ email của bạn.");
			} catch (Exception e) {
				return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
						.body("Mật khẩu mới đã được gửi đến địa chỉ email của bạn.");
			}
		} else {
