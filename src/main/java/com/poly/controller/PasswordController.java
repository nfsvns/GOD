...
public class PasswordController {
	...
	public String changePassword(String oldPassword, String newPassword, String newPasswordAgain,
	...
	@PostMapping("/profile")
	public String updateUserProfile(@ModelAttribute Account updatedAccount,
		...
		if (!imageFile.isEmpty()) {
			try {
				...
				boolean success = acdao.updateProfile(username, updatedAccount.getFullname(), updatedAccount.getEmail(),
						updatedAccount.getPhoto());

				if (success) {
					redirectAttributes.addFlashAttribute("success", "Thông tin cá nhân đã được cập nhật thành công.");
				} else {
					redirectAttributes.addFlashAttribute("error",
							"Không thể cập nhật thông tin cá nhân. Vui lòng kiểm tra lại.");
				}
			} catch (Exception e) {
				redirectAttributes.addFlashAttribute("error", "Không thể tải lên hình ảnh. Vui lòng kiểm tra lại.");
			}
		} else {
			boolean success = acdao.updateProfileWithoutPhoto(username, updatedAccount.getFullname(),
	...
	public String updateAddressToProfile(Model model, RedirectAttributes redirectAttributes,
			@RequestParam(value = "address2", required = false) Integer address2,
	...
	public String deleteAddressFromProfile(@RequestParam(value = "address2", required = false) Integer address2,
			RedirectAttributes redirectAttributes) {
		if (address2 == null) {
